...
export function UserCard({ displayUser }) {
  ...
  const handleSubmit = async () => {
    ...
    const imageurl = profileImage
      ? await uploadImage(profileImage, user.uid)
    ...
    <Card sx={{ marginBottom: "10px" }}>
      <CardContent>
        {displayUser.profileImage && (
          <UserCardAvatar src={displayUser.profileImage} />
        )}
        {!displayUser.profileImage && (
          <Avatar sx={{ width: 200, height: 200, marginInline: "auto" }}>
        ...
        {isEditing ? (
          ...
          <>
            ...
            <Typography variant="body2" color="text.secondary">
              {displayUser?.joined
                ? `Joined ${new Date(displayUser.joined).toLocaleString(
                    "en-US",
                    {
                      month: "long",
                      day: "numeric",
